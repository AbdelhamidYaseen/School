...
namespace LaboOefeningen
{
    public class TextCellMetKomma
    {
        ...
        public static void BerekenEnToonRooster(string[] rooster) {
            ...
            for (int i = 0; i < rooster.Length; i++) {
                Console.Write("|");
                string inhoud = berekendRooster[i];
                if (IsGetal(inhoud)) {
                    double inhoudAlsGetal = Convert.ToDouble(inhoud);
                    inhoud = Convert.ToString(Math.Round(inhoudAlsGetal,2));
                }
                Console.Write(inhoud.PadRight(10).Substring(0,10));
            }
            Console.WriteLine("|");
        }
